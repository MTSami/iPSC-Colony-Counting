...
from . import utils
from .run_maxima import *



### Local Maxima parameters TODO: Move to argparse
CONTRAST_FACTOR = 3
LOCAL_MAXIMA_MIN_DIST = 8
LOCAL_MAXIMA_THRESH = 128
RADIUS = 0
...
if testset.there_is_gt:
    # Prepare Judges that will compute Precison/Recall as funtion of r (radii) and th (threshold, tau)
    judges = []
...
for batch_idx, (imgs, dictionaries) in tqdm(enumerate(testset_loader),
    ...
    # print("Image Name: ", dictionaries[0]['filename'])
    ...
    if testset.there_is_gt:
        ...
        target_count = [dictt['count'].to(device)
                        for dictt in dictionaries]
        ### Sami edits
        target_mask = [dictt['mask'].to(device)
                        for dictt in dictionaries]
    ...
    if testset.there_is_gt:
        ...
        ### Sami edits
        target_mask = target_mask[0].to(device_cpu).numpy().astype("uint8")
        target_mask = np.squeeze(target_mask, 0)
        # print(target_mask.shape) ####(256, 256)
    ...
    # Feed forward
    with torch.no_grad():
        # est_maps, est_masks, est_count = model.forward(imgs)
        
        ## Sami Edits#############################################
        est_maps, est_masks = model.forward(imgs)
        ##########################################################
    ...
    ###Sami edits: Save segementataion masks##############################
    est_mask_np = est_masks[0, :, :].to(device_cpu).numpy()
    # print("old est_mask_np", est_mask_np.shape)
    # est_mask_np = np.squeeze(est_mask_np, 0)
    # print("new est_mask_np", est_mask_np.shape)
    est_mask_np_origsize = \
        skimage.transform.resize(est_mask_np,
                                 output_shape=origsize,
                                 mode='constant')
    #######################################################################
    ...
    ##Debug### np array
    # print("Pred Sal Map: ", type(est_map_np_origsize.shape) )
    # print("Pred Sal Map Max: ", np.max(est_map_np_origsize))
    # print("Pred Sal Map Min: ", np.min(est_map_np_origsize))
    ...
    for t, tau in enumerate(args.taus):
        ...
        ######SAMI EDITS####################################################
        est_map_origsize_int = ((est_map_np_origsize)/(est_map_np_origsize.max()/255.0)).astype("uint8")

        centroids_wrt_orig = run_local_maxima(est_map_origsize_int, 
                                              CONTRAST_FACTOR,
                                              LOCAL_MAXIMA_MIN_DIST,
                                              LOCAL_MAXIMA_THRESH,
                                              RADIUS)
        est_count_int = centroids_wrt_orig.shape[0]
        # print("est_count_int: ", est_count_int)
        
        ######SAMI EDITS####################################################
        # Save raw map to disk
        ...
        est_map_np_origsize_normalized = (est_map_np_origsize)/(est_map_np_origsize.max()/255.0)
        # print("RAW MAP shape:", est_map_np_origsize_normalized.shape)
        # print("RAW MAP max:", np.max(est_map_np_origsize_normalized))
        # print("RAW MAP min:", np.min(est_map_np_origsize_normalized))
        ...
        cv2.imwrite(os.path.join(args.out,
                                 ...
                                 f'tau={round(tau, 4)}',
                                 dictionaries[0]['filename']),
                    est_map_np_origsize_normalized)
        
        # Save seg mask to disk
        os.makedirs(os.path.join(args.out,
                                 'intermediate',
                                 'seg_mask'), exist_ok=True)
                                 
        est_mask_np_origsize_normalized = (est_mask_np_origsize)/(est_mask_np_origsize.max()/255.0)
        
        cv2.imwrite(os.path.join(args.out,
                                 'intermediate',
                                 'seg_mask',
                                 dictionaries[0]['filename']),
                    est_mask_np_origsize_normalized)
        ##################################################################
        ...
        if args.evaluate:
            ...
            for judge in judges:
                ...
                judge.feed_mask(est_mask_np, target_mask)
