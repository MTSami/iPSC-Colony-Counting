...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes,
        ...
        if self.ultrasmall:
            ...
            # Segment Decoder
            self.up1_seg = up(768, 128)
            self.up2_seg = up(256, 64)
            self.up3_seg = up(128, 64, activ=False)
        else:
            ...
            # Segment Decoder
            self.up1_seg = up(1024, 512)
            self.up2_seg = up(1024, 512)
            self.up3_seg = up(1024, 512)
            self.up4_seg = up(1024, 512)
            self.up5_seg = up(1024, 256)
            self.up6_seg = up(512, 128)
            self.up7_seg = up(256, 64)
            self.up8_seg = up(128, 64, activ=False)
        ...
        self.out_nonlin_seg  = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        if self.do_segmentation and self.ultrasmall:
            bottle_neck = x4
            x_seg = self.up1_seg(bottle_neck, x3)
            x_seg = self.up2_seg(x_seg, x2)
            x_seg = self.up3_seg(x_seg, x1)
            
        else:
            bottle_neck = x9
            x_seg = self.up1_seg(bottle_neck, x8)
            x_seg = self.up2_seg(x_seg, x7)
            x_seg = self.up3_seg(x_seg, x6)
            x_seg = self.up4_seg(x_seg, x5)
            x_seg = self.up5_seg(x_seg, x4)
            x_seg = self.up6_seg(x_seg, x3)
            x_seg = self.up7_seg(x_seg, x2)
            x_seg = self.up8_seg(x_seg, x1)
            
        x_seg = self.outc_seg(x_seg)
        x_seg = self.out_nonlin_seg(x_seg)
        x_seg = x_seg.squeeze(1)
        ##########################################################
        ...
        #     x_flat = x_seg.view(batch_size, -1)
