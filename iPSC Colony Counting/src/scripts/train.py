...
loss_focal = losses.FocalLoss(resized_height = args.height,
                              resized_width = args.width, 
                              device = device)
...
while epoch < args.epochs:
    ...
    for batch_idx, (imgs, dictionaries) in enumerate(iter_train):
        ...
        ##SAMI edits###############################################
        target_maps = [dictt['map'].to(device)
                            for dictt in dictionaries]
 
        target_seg_masks = [dictt['mask'].to(device)
                            for dictt in dictionaries]
        
        # print("target_seg_masks]", type(target_seg_masks))
        # print("target_seg_masks_type]", type(target_seg_masks[0]))
        # print("target_seg_masks_content]", target_seg_masks[0])
        ###########################################################
        ...
        ##SAMI edits###############################################
        target_maps = torch.stack(target_maps)
        target_maps = torch.squeeze(target_maps, 1)
        
        target_seg_masks = torch.stack(target_seg_masks)
        target_seg_masks = torch.squeeze(target_seg_masks, 1)

        # One training step
        optimizer.zero_grad()
        
        # est_maps, est_masks, est_counts = model.forward(imgs)
        ...
        term1 = loss_focal.forward(est_maps, target_maps)
        term2 = loss_BCE(est_masks, target_seg_masks)
        
        # print("est_masks", est_masks.size())
        # print("target_seg_masks", target_seg_masks.size())
        
        # print("est_maps", est_maps.size())
        # print("target_maps", target_maps.size())
        ...
        iter_train.set_postfix(running_avg=f'{round(running_avg.avg/2, 1)}')
        ...
        if time.time() > tic_train + args.log_interval:
            ...
            log.train_losses(terms=[term1, term2, loss / 2, running_avg.avg / 2],
                             iteration_number=epoch +
                             batch_idx/len(trainset_loader),
                             terms_legends=['Term1',
                                            'Term2',
    ...
    if not args.val_dir or \
        ...
        if args.save and  args.val_freq % (epoch + 1) == 0:
    ...
    for batch_idx, (imgs, dictionaries) in enumerate(iter_val):
        ...
        ## Find the local maxima of estimated maps to get their centroids 
        ...
        est_map_origsize_int = ((est_map_numpy_origsize)/(est_map_numpy_origsize.max()/255.0)).astype("uint8")
