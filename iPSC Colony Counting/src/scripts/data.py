...
class CSVDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.there_is_gt:
            ...
            ###Sami edits#############################################################
            seg_mask_name = self.csv_df.iloc[idx].filename
            seg_mask_name = seg_mask_name.split(".")[0]+"_mask.png"
            # print("seg_mask_name ", seg_mask_name)
            
            seg_mask_abspath = re.sub("images", "masks", self.root_dir)
            seg_mask_abspath = os.path.join(seg_mask_abspath, seg_mask_name)
            # print("seg_mask_abspath ", seg_mask_abspath)
            ##########################################################################
        ...
        ##################SAMI edits#################################################
        seg_gt_mask = cv2.imread(seg_mask_abspath, cv2.IMREAD_GRAYSCALE)
        # print("Raw seg_gt_mask ", seg_gt_mask.shape)

        # Resize
        seg_gt_mask = cv2.resize(seg_gt_mask, (256, 256))
        
        #Threshold to {0, 1} binary mask
        seg_gt_mask = cv2.threshold(seg_gt_mask, 0, 1, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
        # print("Processed seg_gt_mask ", seg_gt_mask.shape)
        #############################################################################
        ...
        if self.there_is_gt:
            ...
            # str -> lists for gt prob map
            ##################SAMI edits#################################################
            dictionary['map'] = eval(dictionary['map'])
            dictionary['map'] = [
                list(loc) for loc in dictionary['map']]
            #############################################################################
            ...
            with torch.no_grad():
                ...
                ##################SAMI edits#################################################
                dictionary['map'] = torch.tensor(
                    [dictionary['map']], dtype=torch.get_default_dtype())
                
                #Sami: Convert np segmentation gt mask to torch tensor
                dictionary['mask'] = torch.from_numpy(seg_gt_mask).float()
                dictionary['mask'] = torch.unsqueeze(dictionary['mask'], 0)
                
                
                # print("dictionary['map']", type(dictionary['map']))
                # print("dictionary['map']", dictionary['map'].size())
                # print("dictionary['map']", dictionary['map'].dtype)
                
                # print("dictionary['mask']", type(dictionary['mask']))
                # print("dictionary['mask']", dictionary['mask'].size())
                # print("dictionary['mask']", dictionary['mask'].dtype)
                #############################################################################
